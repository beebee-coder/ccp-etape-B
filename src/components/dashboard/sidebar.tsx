...
const navItems = [
  ...
  { href: "/admin/pipeline", label: "Pipeline", icon: GitBranch, roles: ["admin"] },
...
export function DashboardSidebar({ role }: { role: "admin" | "chef-de-quart" | "chef-de-bloc" | "rondier" }) {
  ...
  return (
    <aside className="relative flex h-full w-64 flex-col overflow-hidden">
      ...
      <div className="relative flex h-16 items-center gap-3 px-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-[10px] text-muted-foreground/70 uppercase tracking-wider">Opérationnel</span>
          </div>
        </div>
      ...
      {/* ── Navigation ── */}
      <nav className="relative flex-1 space-y-0.5 overflow-y-auto px-3 py-3 scrollbar-none">
        {items.map((item, idx) => {
