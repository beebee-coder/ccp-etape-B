...
import type { SensorReading } from "@/lib/embedded/device-service";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="group relative rounded-xl border border-border/50 bg-card overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10 hover:border-blue-500/30">
                ...
                <>
                  <div className="relative w-full h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-[11px] text-primary/70 font-bold tracking-wider">{data.camera.resolution}</p>
                      <p className="text-[10px] text-muted-foreground mt-0.5">{data.camera.fps} fps · 30 ms</p>
                    </div>
                  </div>
                  {data.camera.motionDetected && (
            ...
            <div className="p-3.5 border-t border-border/50 bg-gradient-to-r from-blue-500/5 to-transparent">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={data.camera.active ? "default" : "secondary"} className={`text-[9px] font-bold ${
                  data.camera.active ? "bg-blue-500/15 text-blue-700 dark:text-blue-400 border-blue-500/30" : ""
                }`}>
                  {data.camera.active ? "Active" : "Inactive"}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div className="group relative rounded-xl border border-border/50 bg-card p-5 transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/10 hover:border-emerald-500/30">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge variant={data.microphone.active ? "default" : "secondary"} className={`text-[9px] font-bold ${
                data.microphone.active ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30" : ""
              }`}>
                {data.microphone.active ? "Actif" : "Muet"}
              </Badge>
            </div>
            {data.microphone.active ? (
              <>
                <div className="space-y-3">
                  <div className="flex items-end gap-1 h-12 bg-muted/30 rounded-lg p-2">
                    {Array.from({ length: 24 }).map((_, i) => {
                      const h = Math.max(4, data.microphone.level * (0.4 + Math.random() * 0.6));
                      const isHigh = h > 70;
                      const isMid = h > 40;
                      return (
                        <div
                  ...
                  <div className="flex items-center justify-between px-1">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-mono font-bold ${
                        data.microphone.level > 70 ? "text-red-500" : data.microphone.level > 40 ? "text-amber-500" : "text-emerald-500"
                      }`}>
                        {Math.round(data.microphone.level)}%
                      </span>
                    </div>
                  </div>
                </div>
                {data.microphone.noiseDetected && (
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${
                  data.temperature.alert ? "bg-red-500/15" : "bg-violet-500/15"
                }`}>
                  <Thermometer className={`h-4 w-4 ${
                    data.temperature.alert ? "text-red-500" : "text-violet-600 dark:text-violet-400"
                  }`} />
                </div>
                <span className="text-[11px] text-muted-foreground font-bold uppercase tracking-wider">Température</span>
              </div>
              <Badge variant={data.temperature.alert ? "destructive" : "secondary"} className={`text-[9px] font-bold ${
                data.temperature.alert ? "" : "bg-violet-500/15 text-violet-700 dark:text-violet-400 border-violet-500/30"
              }`}>
                {data.temperature.alert ? "Alerte" : "Normal"}
              </Badge>
            </div>
            {data.temperature.active ? (
              <>
                <div className="text-center py-3 relative">
                  <div className="inline-flex items-baseline gap-1">
                    <span
                      className={`text-4xl font-black font-mono tabular-nums tracking-tight transition-colors duration-500 ${
                        data.temperature.alert ? "text-red-500" : "text-foreground"
                      }`}
                    >
                      {data.temperature.current}
                    </span>
                    <span className="text-lg text-muted-foreground font-medium">°{data.temperature.unit}</span>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2.5">
                    <div className="rounded-xl bg-muted/40 p-2.5 text-center border border-border/30">
                      ...
                      <span className="text-sm font-bold text-foreground mt-0.5">{data.temperature.min}°{data.temperature.unit}</span>
                    </div>
                    <div className="rounded-xl bg-muted/40 p-2.5 text-center border border-border/30">
                      <span className="text-[9px] text-muted-foreground block font-medium uppercase tracking-wider">Max</span>
                      <span className="text-sm font-bold text-foreground mt-0.5">{data.temperature.max}°{data.temperature.unit}</span>
                    </div>
                  </div>
                    ...
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${
                        data.temperature.current > 30
                          ...
                          : data.temperature.current > 25
                            ? "bg-gradient-to-r from-amber-500 to-amber-400"
                            : "bg-gradient-to-r from-violet-500 to-violet-400"
                      }`}
                      style={{
                        width: `${Math.min(100, ((data.temperature.current - data.temperature.min) / (data.temperature.max - data.temperature.min)) * 100)}%`,
                      }}
                    />
                  </div>
                  <div className="flex justify-between mt-1.5">
                    <span className="text-[9px] text-muted-foreground/70 font-mono">{data.temperature.min}°</span>
                    <span className="text-[9px] text-muted-foreground/70 font-mono">{data.temperature.max}°</span>
                  </div>
                </div>
              </>
            ) : (
