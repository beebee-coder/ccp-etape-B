...
function parsePrismaSchema(filePath: string): IntrospectionResult | null {
  ...
  logger.debug(`${label} Prisma schema loaded (${content.length} bytes)`, { filePath });
  ...
  while ((modelMatch = modelRegex.exec(content)) !== null) {
    ...
    logger.debug(`${label} Parsing Prisma model "${modelName}"`, { modelName });
    ...
    for (const line of lines) {
      if (line.startsWith("@@")) {
        if (line.startsWith("@@index")) {
          ...
          if (idxMatch) {
            const fields = idxMatch[1].split(",").map((f) => f.trim().replace(/"/g, ""));
            indexes.push(`${modelName}_${fields.join("_")}_idx`);
          }
        }
        continue;
      }
      ...
      if (colMatch && !colMatch[1].startsWith("_")) {
        ...
        const isForeign = rest.includes("@relation");
        const unique = rest.includes("@unique");
        const defaultValue = rest.includes("@default")
          ? rest.match(/@default\(([^)]+)\)/)?.[1] ?? null
          : null;
        const nullable = !rest.includes("@db") && !unique && !isPrimary && !rest.includes("!");

        let references: string | null = null;
        if (isForeign) {
          const relationMatch = rest.match(/@relation\(fields:\s*\[([^\]]+)\],\s*references:\s*\[([^\]]+)\]\)/);
          if (relationMatch) {
            references = `${modelName}(${relationMatch[2].replace(/"/g, "").trim()})`;
            relations.push(references);
          }
        }
        ...
        columns.push({
          name: colName,
          type: mapPrismaTypeToSql(colType),
          nullable,
          defaultValue,
          isPrimary,
          isForeign,
    ...
    tables.push({
      name: tableName,
      description: `Prisma model ${modelName} → table ${tableName}`,
      columns,
      indexes,
      relations,
    });
  ...
  logger.info(`${label} Parsed ${tables.length} models from Prisma schema`, { tableCount: tables.length });
